...
namespace Fan.ResMgr
{
    public class ResObj
    {
        ...
        public byte MaxTryLoadNum//最多尝试下载的次数。0为限制尝试
        {
            get { return _maxTryLoadNum; }
        }

        public ResObj(string subPath, byte prior, byte maxTryLoadNum)
        {
            _subPath = subPath;
            _prior = prior;
        ...
        public ResObj(string subPath) : this(subPath, 0, 0) { }
        public ResObj(string subPath, byte prior) : this(subPath, prior, 0) { }
    ...
    public sealed class ResLoader : MonoBehaviour
    {
        ...
        private const float ERR_TIMEOUT = 10f;//10s秒进度没有变化要重新开始加载
        ...
        private System.Collections.IEnumerator LoadFile()
        {
            ...
            if (ResMgr.instance.localManifest.assetDic.TryGetValue(_resObj.SubPath, out localAssetInfo) 
                ...
                && localAssetInfo.Equals(remoteAssetInfo) && ResUtil.ExistsInLocal(_resObj.SubPath, out localPath))
            {
                _formLocal = true;
                AssetBundleCreateRequest abcr = AssetBundle.CreateFromMemory(System.IO.File.ReadAllBytes(localPath));
                while(!abcr.isDone)
                {
...
//#endif
                    ...
                    {
                        Fan.MDebug.LogError(string.Format("{0}加载失败…第{1}次尝试加载", _resObj, _reLoadNum));
                ...
                else
                {
                    ...
                    if (_assetBundle == null)
                        Fan.MDebug.LogError("you must load assetBundle file with resloader" + _resObj);
...
//#endif
            ...
            {
                WWW www = new WWW(ResUtil.GetRemotePath(_resObj.SubPath));
...
//#endif
                ...
                {
                    ResUtil.Save2Local(_resObj.SubPath, www.bytes);
                    ...
                    if (ResMgr.instance.remoteManifest.assetDic.TryGetValue(_resObj.SubPath, out remoteAssetInfo))
                    {
                        ...
                        ResUtil.SaveManifest2Local(ResMgr.instance.localManifest);
                    }
                    _state = EResLoaderState.Complete;
                    ...
                    www = null;
                    if (_assetBundle == null)
                        Fan.MDebug.LogError("you must load assetBundle file with resloader");
                    CurrentProgress = 1f;
                    if (completeHandler != null)
                        completeHandler(this);
//#if !UNITY_EDITOR
